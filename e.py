...
<script>
    // VERİTABANI DOĞRUDAN koda GÖMÜLDÜ (CORS Hatası Veremez)
...
def generate_all():
    ...
    for json_file, cfg in json_files_data.items():
        ...
        if os.path.exists(json_file):
            try:
                with open(json_file, "r", encoding="utf-8") as jf:
                    ...
                    # Eğer veri liste halinde geldiyse dict yapısına çevir
                    if isinstance(raw_data, list):
                        for item in raw_data:
                            if "radio_serial" in item and "code" in item:
                                db_data[item["radio_serial"]] = item["code"]
        ...
        created += 1
        print(
            f"[BAŞARILI] {cfg['html_name']} oluşturuldu. ({len(db_data)} adet kod entegre edildi)"
        )
